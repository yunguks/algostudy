...
2 초	512 MB	5668	2108	1591	37.710%
문제
...
농부 존은 1번 동영상과 2번 동영상이 USADO 3을 가지고, 2번 동영상과 3번 동영상이 USADO 2를 가지고,
 ...
 이것에 기반해서 1번 동영상과 3번 동영상의 USADO는 min(3,2)=2가 되고,
  1번 동영상과 4번 동영상의 USADO는 min(3,4)=3이 되고, 3번 동영상과 4번 동영상의 USADO는 min(2,4)=2가 된다.

농부 존은 K=1일 때 2번 동영상, K=3일 때 1번 동영상,
 K=4일 때 1번 동영상을 보면 각각 몇 개의 동영상이 추천될까 궁금해하고 있다.
...
if __name__=="__main__":
    ...
    for _ in range(Q):
        ...
        visit[V]=1
        q = deque()
        q.append([1000000001,V])
        while len(q):
            r,next = q.popleft()
            for usado,node in nodes[next]:
                if visit[node] == 0 and min(r,usado) >=K:
                    count+=1
                    visit[node]=1
                    q.append([min(r,usado),node])
        print(count)

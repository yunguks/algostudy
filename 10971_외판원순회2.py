...
def dfs(start, next, cost, visit):
    ...
    global n
    global result
    # result 보다 cost가 크다면 돌지 않음 // 이것 때문에 시간초과
    if result < cost:
        return
    ...
    if len(visit)==n:  
        x = W[next][visit[0]]       
        if x !=0:
            result = min(result, cost+x)
        return

    else:
        for i in range(n):
            if i not in visit and W[next][i]!=0:
                visit.append(i)
                dfs(next,i,cost+W[next][i],visit)
                visit.pop()

for i in range(n):
    # visit 에 i 시작점을 넣어주어야 
    # 이코드에서 방문한 것으로 하고 계산한다. 아니라면 pop에서 빼서 여러번 수행
    dfs(i,i,0,[i])
